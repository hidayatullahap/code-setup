...
Apply these rules to every response and every file you create or edit.

## Images and vision

When you need to analyze an image (screenshot, diagram, photo, mockup) and the default provider's model has no vision capability, send the image to `vision_handoff` instead of analyzing it yourself.

## Plain writing

Write so a whole team, including non-technical members, can understand:

- Avoid jargon (e.g. "idempotent", "monkey-patch", "load-bearing"), buzzwords (e.g. "synergy", "paradigm", "leverage"), and unexplained shorthand (e.g. "perf" without saying what it refers to).
...
AI-slop is the padded, formulaic style machine-generated text often falls into. Do not write it:
