...
- **Phase-based prompt composition** via `before_agent_start`: fresh runs get workflow + output rules + anti-slop + commitment + polish; tweak turns (when `index.html` exists) get revision-tweaks + output rules + anti-slop.
...
- No `index.html` at `ctx.cwd` → fresh run.
- `index.html` exists → tweak turn.
